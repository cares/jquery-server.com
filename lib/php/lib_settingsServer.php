...
if (file_exists ( 'lib_general.php' )) {
	require_once ('lib_general.php');
} else {
	if (file_exists ( './lib/php/lib_general.php' )) {
		require_once ('./lib/php/lib_general.php');
	} else {
		trigger_error ( basename ( __FILE__, '.php' ) . "-> could not include library lib_general.php, it should be on top of every file.php", E_USER_ERROR );
	}
}
...
if($_REQUEST["get"] == "settings")
{
	...
	/* ======================= DEVELOPMENT */
	$settings["debug_mode"]					= config::get('debug_mode');
